...
export const fetchBlocksForNode = createAsyncThunk(
  "nodes/checkBlockStatus",
  async (node: Node) => {
...
export const checkBlockStatus = createAsyncThunk(
  ...
  async (nodes: Node[], thunkAPI) => {
    ...
    nodes.forEach((node) => {
      dispatch(fetchBlocksForNode(node));
    });
  }
);
...
export const nodesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchBlocksForNode.pending, (state, action) => {
      const node = state.list.find((n) => n.url === action.meta.arg.url);
      if (node) {
        node.loading = true;
      }
    ...
    builder.addCase(fetchBlocksForNode.fulfilled, (state, action) => {
      const node = state.list.find((n) => n.url === action.meta.arg.url);
      if (node) {
    ...
    builder.addCase(fetchBlocksForNode.rejected, (state, action) => {
      const node = state.list.find((n) => n.url === action.meta.arg.url);
      if (node) {
        node.loading = false;
        node.isError = true;
